...
function ageFish(startingAges, numDays) {
  ...
  let numPerDay = _.range(0, 8).map(day => startingAges.filter(age => age === day).length);
  
  for (let i = 0; i < numDays; i++) {
    let newNumPerDay = _.range(0, 8).map(day => numPerDay[day + 1]); // Move every fish forward one day
